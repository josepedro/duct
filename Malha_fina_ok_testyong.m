...
Nr, distance, growth_delta, point_x, point_y);
...
imagesc(flip(sigma_mat9_esquerda(:,:,1)));
...
%xl = [250 250];
%yl = [1 50];
xl = [2 30+100*6];%Lt=230 voxel; Ltubo=200 voxel
yl = [20*2 20*2];
...
% Construindo chirp
a=yl(2);
total_time = round(15*Mc*sqrt(3)); % meia hora = 20*Mc*sqrt(3)
final_time_chirp = total_time - round(2*xl(2)*sqrt(3));
...
%         imagesc(flip(vorticidade))
    ...
    x_probe=570; y_probe=2:39;
    
    % Face data
    
    pressure1(ta) = (mean(rho(y_probe, x_probe)-1))*cs2;
    particle_velocity1(ta) = mean(ux(y_probe, x_probe));
    
    particle_velocity3(ta) = mean(ux(y_probe, 330));
    pressure3(ta) = (mean(rho(y_probe, 330)-1))*cs2;
    % point data
    
    particle_velocity2(ta) = mean(ux(y_probe, 600));
    pressure2(ta) = (mean(rho(y_probe, 600)-1))*cs2;
