...
%xl = [250 250];
%yl = [1 50];
xl = [29 29 30+100*5];%Lt=230 voxel; Ltubo=200 voxel
yl = [1 20*2+1 20*2+1];
...
for ta = 1 : total_time
    ...
    f(vec5_duto)=G(vec6_duto);
    f(vec7_duto)=G(vec8_duto);
    f(vec6_duto)=G(vec5_duto);
    f(vec8_duto)=G(vec7_duto);
...
%         imagesc(flip(vorticidade))
    ...
    x_probe=280; y_probe=35;
    % Face data
    
    pressure1(ta) = (mean(rho(2:38, x_probe)-1))*cs2;
    particle_velocity1(ta) = mean(ux(2:38, x_probe));
    
    particle_velocity3(ta) = ux(y_probe, 500);
    pressure3(ta) = (rho(y_probe, 500)-1)*cs2;
    % point data
