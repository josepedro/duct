...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
omega = 1.75;                                      % Relaxation frequency
...
%xl = [250 250];
%yl = [1 50];
xl = [29 29 100*5];%Lt=230 voxel; Ltubo=200 voxel
yl = [1 20*2+1 20*2+1];
...
final_time_chirp = total_time - round(2*xl(3)*sqrt(3));
times = 0 : final_time_chirp;
initial_frequency = 0*cs/(2*pi*a);
frequency_max_lattice = 10*cs/(2*pi*a);
...
pressure5(1:total_time) = 0;
pressure6(1:total_time) = 0;
pressure7(1:total_time) = 0;
pressure8(1:total_time) = 0;
...
particle_velocity5(1:total_time) = 0;
particle_velocity6(1:total_time) = 0;
particle_velocity7(1:total_time) = 0;
particle_velocity8(1:total_time) = 0;
...
for ta = 1 : total_time
    ...
    if ta <= length(source_chirp)
        density_source = rho_l + 0.00001*source_chirp(ta);
        Ux_t = (0.00001*source_chirp(ta))/sqrt(3);
    else
...
%         imagesc(flip(vorticidade))
    ...
    pressure1(ta) = (mean(rho(p_y,p1_x)-1))*cs2;
    particle_velocity1(ta) = mean(ux(p_y, p1_x));
    
    pressure2(ta) = (mean(rho(p_y, p2_x)-1))*cs2;
    particle_velocity2(ta) = mean(ux(p_y, p2_x));
    
    pressure3(ta) = (mean(rho(p_y, p3_x)-1))*cs2;
    particle_velocity3(ta) = mean(ux(p_y, p3_x));
    
    pressure4(ta) = (mean(rho(p_y, p4_x)-1))*cs2;
    particle_velocity4(ta) = mean(ux(p_y, p4_x));
    
     pressure5(ta) = (mean(rho(p_y,p5_x)-1))*cs2;
    particle_velocity5(ta) = mean(ux(p_y, p5_x));
    
    pressure6(ta) = (mean(rho(p_y, p6_x)-1))*cs2;
    particle_velocity6(ta) = mean(ux(p_y, p6_x));
    
    pressure7(ta) = (mean(rho(p_y, p7_x)-1))*cs2;
    particle_velocity7(ta) = mean(ux(p_y, p7_x));
    
    pressure8(ta) = (mean(rho(p_y, p8_x)-1))*cs2;
    particle_velocity8(ta) = mean(ux(p_y, p8_x));
...
%     % Face data
%     
%     pressure1(ta) = (mean(rho(2:39, x_probe)-1))*cs2;
%     particle_velocity1(ta) = mean(ux(2:39, x_probe));
%     
...
%%
% Postprocessing
fft_pressure = fft(pressure8);
fft_particle_velocity = fft(particle_velocity8);
...
plot(ka,imag(Zr),'--red')
axis([0 3 0 1]);
ylabel('Impedância, Zr','FontSize',20);
...
axis([0 3 0 1]);
ylabel('Coeficiente de Reflexão, Rr','FontSize',20);
xlabel('Numero de Helmholtz, ka','FontSize',20);
...
% axis([0 3 0 1]);
% ylabel('Impedância, Z_end','FontSize',20);
% xlabel('Numero de Helmholtz, ka','FontSize',20);
% legend('Real(Analitico)', 'Imaginaria(Analitico)','Real(LBM)', 'Imaginaria(LBM)');
% hold off
% 
